  ...
  <div id="home" class="bg-white">
    <div class="h-screen">
      <div class="absolute top-[100px] md:top-[200px] w-full z-10">
        <p class="text-[1rem] md:text-[2rem] xl:text-[3.2rem] font-black mx-auto w-[90%] md:w-[65%]">
          {{ $t('intro') }}
        </p>
      </div>
    ...
    <div id="container"
      ...
      <div id="first-paragraph" class="flex flex-row flex-wrap whitespace-pre-wrap">
        <p v-for="(word, index) in firstParagraphWords" :key="index" :style="{ opacity: getOpacityFirstParagraph(index) }"
          class="whitespace-pre-line mr-1 mb-2 md:mr-4 md:mb-4" v-html="word"></p>
      </div>
      <br />
      <div id="second-paragraph" class="flex flex-row flex-wrap whitespace-pre-wrap">
        <p v-for="(word, index) in secondParagraphWords" :key="index"
          :style="{ opacity: getOpacitySecondParagraph(index) }" class="whitespace-pre-line mr-1 mb-2 md:mr-4 md:mb-4"
          v-html="word"></p>
      </div>
      <br />
      <div :ref="`container`" class="flex flex-row flex-wrap whitespace-pre-wrap">
        <p v-for="(word, index) in thirdParagraphWords" :key="index" :style="{ opacity: getOpacityThirdParagraph(index) }"
          :ref="`pTag${index}`" class="whitespace-pre-line mr-1 mb-2 md:mr-4 md:mb-4" v-html="word"></p>
      </div>
    </div>
...
export default {
  ...
  methods: {
    ...
    handleScroll(event) {
      this.firstParagraphHeight = document.getElementById("first-paragraph").offsetHeight;
      this.secondParagraphHeight = document.getElementById("second-paragraph").offsetHeight;
    ...
    getOpacityFirstParagraph(index) {
      ...
      if (this.scrollPos - this.windowHeight + adjustFirstParagrah < opacityChangeStart) {
        return 0.2;
    ...
    getOpacitySecondParagraph(index) {
      ...
      if (this.scrollPos - this.firstParagraphHeight - this.windowHeight + adjustSecondParagraph < opacityChangeStart) {
        return 0.2;
      } else if (this.scrollPos > opacityChangeEnd) {
        return 1;
      }
    ...
    getOpacityThirdParagraph(index) {
      ...
      if (this.scrollPos - (this.secondParagraphHeight + this.firstParagraphHeight) - this.windowHeight - adjustThirdParagraph < opacityChangeStart) {
        return 0.2;
      } else if (this.scrollPos > opacityChangeEnd) {
        return 1;
      }
