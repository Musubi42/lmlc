<template>
  <div class="bg-bleu-ciel/70 h-16 flex bg-opacity-70">
    ...
    >
      <img class="h-4/5 aspect-square flex" :src="songMetadata ? songMetadata.thumbnail : ''">
    ...
    >
      ...
      >
        <div class="z-10">
          <iconsSoundOn v-if="isSound" class="w-[24px]" />
          <iconsSoundOff v-else class="w-[24px]" />
        </div>
  ...
  methods: {
    async toggleAudio() {
      if (!this.audioSource) {
        ...
        const audioResponse = await fetch(`${this.APIStreamAudioBaseUrl}/audio/${this.trackID}`);
